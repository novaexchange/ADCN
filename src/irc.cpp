...
// Copyright (c) 2011-2012 AsiaDigiCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("AsiaDigiCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AsiaDigiCoinIRCTEST0\r");
            Send(hSocket, "WHO #AsiaDigiCoinIRCTEST0\r");
        } else {
            // randomly join #AsiaDigiCoinIRC00-#AsiaDigiCoinIRC99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #AsiaDigiCoinIRC%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #AsiaDigiCoinIRC%02d\r", channel_number).c_str());
        }
