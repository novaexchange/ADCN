...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AsiaDigiCoin-Data";
#else
    fs::path pathRet;
    ...
    return pathRet / "AsiaDigiCoin-Data";
#else
    // Unix
    return pathRet / ".AsiaDigiCoin-Data";
#endif
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 15 * 60) // AsiaDigiCoin: changed maximum adjust to 15 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
